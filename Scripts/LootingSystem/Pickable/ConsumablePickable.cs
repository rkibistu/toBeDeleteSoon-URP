...
public class ConsumablePickable : PickableItem {
    ...
    public void ReloadDamageGrenades(PlayerRef playerRef) {

        AgentStateMachine agent = null;
        if (playerRef.IsNone) {
            // playerRef is none when a RPC is called by the host
            agent = RoomPlayer.LocalRoomPlayer.ActiveAgent;
        }
            ...
            agent = Context.Instance.Gameplay.Players[playerRef].ActiveAgent;
            //Debug.LogError("Do you ever want this ?");
        }

        agent?.Weapons.CurrentWeapon.DamageGrenadeMagazine.AddAmmoToWeapon(1);
    }

    public void ReloadSmokeGrenades(PlayerRef playerRef) {
        ...
        agent?.Weapons.CurrentWeapon.SmokeGrenadeMagazine.AddAmmoToWeapon(1);
    }
