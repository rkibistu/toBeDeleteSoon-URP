...
public class Health : NetworkBehaviour, IHitTarget, IHitInstigator {
    ...
    GameObject IHitTarget.GameObject => transform.gameObject;
    ...
    private void ApplyHit(ref HitData hitData) {
        ...
        //Debug.Log("Set hit: " + hit.Instigator);
    ...
    private void UpdateVisibleHits() {
        ...
        for (int i = Mathf.Max(_visibleHitCount + 1, oldestHit); i <= _hitCount; i++) {
            ...
            // Debug.Log("vis:   before: " + hit.HealthBefore + "  now: " + CurrentHealth);
            ...
            var hitData = new HitData {
                ...
                IsFatal = (CurrentHealth <= 0 && (hit.HealthBefore - hit.Damage <= 0)),
            };
    ...
    private void OnHitTaken(ref HitData hitData) {
        ...
        if (hitData.InstigatorRef.IsValid == true && hitData.InstigatorRef == Runner.LocalPlayer) {
            var instigator = hitData.Instigator;

            if (instigator == null) {
                var playerObject = Runner.GetPlayerObject(hitData.InstigatorRef);
                var agent = playerObject != null ? playerObject.GetComponent<RoomPlayer>().ActiveAgent : null;
