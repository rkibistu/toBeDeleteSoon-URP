...
public static class HitUtility {
    ...
    public static HitData ProcessHit(PlayerRef instigatorRef, Vector3 direction, LagCompensatedHit hit, float baseDamage, EHitType hitType, out bool? hitYourself) {
        ...
        //check if target and insigator are the same -> don;t hit yourself!

        if (target.GameObject.GetComponent<NetworkObject>().InputAuthority == instigatorRef) {
        ...
        if (hitType == EHitType.None) {
            // Some projectiles should not do damage!
            return default;
        }
        if (instigatorRef == PlayerRef.None) {
            return default;
        }
