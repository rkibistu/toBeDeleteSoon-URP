...
[OrderBefore(typeof(RoomPlayer))]
public class PlayerInput : NetworkBehaviour, INetworkInput, IBeforeUpdate, IBeforeTick
{
    // PUBLIC MEMBERS
    [Networked]
    ...
    /// <summary>
    /// returneaza true de mai multe ori in timpul unui tick cand apesi un buton
    ...
    /// <summary>
    /// returneaza true o singura data in timpul unui tick cand apesi un buton
    ...
    public void LockCursour() {
        ...
        Context.Instance.SceneInput.RequestCursorLock();   
    }
    
    public void UnlockCursour() {
    ...
    public override void Spawned() {
        ...
        if (Runner.LocalPlayer == Object.InputAuthority) {
            ...
            //Asta ar trebui folosit in timp ce jocul e in derulare. Si dezactivata cat avem un meniu activ
            //Context.Instance.SceneInput.RequestCursorLock();
        }
    }
    ...
    void IBeforeTick.BeforeTick() {
        ...
        //Functia GetInput returneaza True si itnaorce in parametru input, ultimul input setat in functia OnInput (networkInput.set(input))
    ...
    //functia asta poate fi aoelata de mai  multe ori intre 2 apeluri ale FixedUpdateNetwork()
    // aici se preia efectiv inputul de la player -> se pune in renderInput
    ...
    private void ProcessStandaloneInput() {
        ...
        _renderInput.Buttons.Set(EInputButtons.Test, Input.GetKey(KeyCode.T));
        _renderInput.Buttons.Set(EInputButtons.Sprint, Input.GetKey(KeyCode.LeftShift));
        _renderInput.Buttons.Set(EInputButtons.Slow, Input.GetKey(KeyCode.LeftControl));
        _renderInput.Buttons.Set(EInputButtons.ZoomWeapon, Input.GetKey(KeyCode.Mouse1));
        _renderInput.Buttons.Set(EInputButtons.ThrowGrenade, Input.GetKey(KeyCode.G));
        _renderInput.Buttons.Set(EInputButtons.ThrowSmoke, Input.GetKey(KeyCode.F));
        _renderInput.Buttons.Set(EInputButtons.Interact, Input.GetKey(KeyCode.E));


        

        //PENTRU WEAPON BUTTON
